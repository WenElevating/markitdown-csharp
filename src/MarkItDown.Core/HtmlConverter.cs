...
public sealed class HtmlConverter : IConverter
{
    ...
    public DocumentKind Kind => DocumentKind.Html;

    public bool CanConvert(DocumentConversionRequest request)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            document.Load(request.FilePath);

            RemoveNoise(document);

            var title = NormalizeInlineText(document.DocumentNode.SelectSingleNode("//title")?.InnerText ?? string.Empty);
            var contentRoot = SelectContentRoot(document);
            var blocks = RenderBlockChildren(contentRoot)
                .Where(block => !string.IsNullOrWhiteSpace(block))
                .ToList();

            if (blocks.Count == 0 && !string.IsNullOrWhiteSpace(title))
            {
                blocks.Add($"# {title}");
            }

            var markdown = string.Join($"{Environment.NewLine}{Environment.NewLine}", blocks).Trim();
            return Task.FromResult(new DocumentConversionResult(DocumentKind.Html, markdown, title));
        }
        ...
        {
            throw new ConversionException(ConversionErrorCode.ConversionFailed, "Failed to convert HTML to Markdown.", ex);
        }
    }
    ...
    {
        return document.DocumentNode.SelectSingleNode("//*[@id='__blog-post-container']")
