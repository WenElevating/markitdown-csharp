...
public sealed class PdfConverterTests
{
    ...
    {
        var result = await _converter.ConvertAsync(new DocumentConversionRequest(FixturePath.For("sample.pdf")));
    ...
    {
        var result = await _converter.ConvertAsync(new DocumentConversionRequest(FixturePath.For("table.pdf")));
    ...
    public async Task ConvertAsync_RejectsScannedPdf()
    {
        var exception = await Assert.ThrowsAsync<ConversionException>(() =>
            _converter.ConvertAsync(new DocumentConversionRequest(FixturePath.For("scanned.pdf"))));

        Assert.Equal(ConversionErrorCode.UnsupportedContent, exception.Code);
